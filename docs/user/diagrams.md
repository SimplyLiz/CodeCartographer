...
`cartographer diagram` turns the import graph into a diagram you can paste
...
```
cartographer diagram [OPTIONS]
```

All options are optional — calling `cartographer diagram` with no flags
...
The format controls the **source** cartographer emits. The destination
...
If the required binary isn't on `$PATH`, cartographer errors out with the
...
```bash
cartographer diagram -o architecture.html
```
...
cartographer diagram -o architecture.svg

# DOT → PNG (requires Graphviz)
cartographer diagram --format dot -o architecture.png
```
...
Cartographer picks a subset of nodes to render because whole-project graphs
...
```bash
cartographer diagram --max-nodes 20
```
...
```bash
cartographer diagram --focus src/parser.rs --depth 2
```
...
```bash
cartographer diagram --blast-radius src/api/auth.rs -o blast.svg
```
...
```bash
cartographer diagram --docs-only -o doc-map.html
```
...
- **Dashed / dotted red, orange, yellow edges** — layer violations
  (back-call, skip-call, direct-foreign-import — see `cartographer layers`).
...
```bash
cartographer diagram --cochange-threshold 0.6
```
...
```bash
cartographer diagram --group-by-folder 2 -o folders.svg
```
...
```bash
cartographer diagram --color-by-owner -o ownership.svg
```
...
```bash
cartographer diagram --call-graph src/parser.rs --format ascii
```
...
cartographer diagram --max-nodes 30

# Blast radius of a file, as a shareable image
cartographer diagram --blast-radius src/api/auth.rs -o blast.svg

# Interactive explorer for the whole project, with ownership colours
cartographer diagram --color-by-owner --max-nodes 200 -o architecture.html

# Folder-level bird's-eye view
cartographer diagram --group-by-folder 2 --format dot -o folders.dot

# Doc-map: which docs point to which code
cartographer diagram --docs-only -o doc-map.html

# ASCII tree rooted on the file you're about to edit (read in the terminal)
cartographer diagram --focus src/parser.rs --depth 2 --format ascii

# Function-level call graph of one file
cartographer diagram --call-graph src/parser.rs --format mermaid -o calls.mmd

# Temporal coupling overlay on a focused view
cartographer diagram --focus src/api --cochange-threshold 0.6 -o coupling.svg
```
...
The same renderer is reachable via FFI (`cartographer_render_architecture`)
