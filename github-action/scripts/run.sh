#!/usr/bin/env bash
# Run Nyx.Navigator analysis on the current PR and post a health-delta comment.
#
...
echo "::group::Nyx.Navigator health (HEAD)"
if ! navigator health --json > "${HEALTH_HEAD}" 2>&1; then
  cat "${HEALTH_HEAD}"
  die "navigator health failed"
fi
cat "${HEALTH_HEAD}"
echo "::endgroup::"

if [[ -n "${BASE_SHA:-}" ]]; then
  echo "::group::Nyx.Navigator health --compare ${BASE_SHA}"
  if ! navigator health --compare "${BASE_SHA}" --json > "${HEALTH_COMPARE}" 2>&1; then
...
echo "::group::Nyx.Navigator hotspots"
navigator hotspots --commits "${COMMITS}" --by-author --bus-factor --json \
  > "${HOTSPOTS}" 2>&1 || true
echo "::endgroup::"
...
echo "::group::Nyx.Navigator snapshot"
navigator snapshot save "${HEAD_SHA:0:12}" 2>&1 || true
echo "::endgroup::"
...
build_comment() {
  ...
  cat <<COMMENT
<!-- navigator-action-comment -->
## ${score_icon} Nyx.Navigator Health — \`${score_head}/100\`
COMMENT
...
COMMENT
  ...
  if [[ "${CYCLE_COUNT}" -gt 0 ]]; then
    echo ""
    echo "> ⚠️ **${CYCLE_COUNT} dependency cycle(s) detected.** Run \`navigator health\` locally to see details."
  fi
  if [[ "${VIOLATION_COUNT}" -gt 0 ]]; then
    echo ""
    echo "> ⚠️ **${VIOLATION_COUNT} layer violation(s).** Run \`navigator layers validate\` locally."
  fi
...
<sub>Generated by [Nyx.Navigator](https://github.com/anthropics/navigator) · commit \`${HEAD_SHA:0:7}\`</sub>
COMMENT
}
...
if [[ "${POST_COMMENT}" == "true" && "${EVENT_NAME}" == "pull_request" && -n "${PR_NUMBER:-}" ]]; then
  ...
  MARKER="<!-- navigator-action-comment -->"
